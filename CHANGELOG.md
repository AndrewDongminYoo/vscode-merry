...
- **Terminal reuse setting** (`vscode-merry.reuseTerminal`) — three-way enum: `never` (default), `always`, `ask`. When set to `ask`, a quick-pick dialog prompts each time.
...
- **Install CLI command** (`vscode-merry.installCli`) — opens a terminal and runs `dart pub global activate merry`; re-checks for the CLI after 5 seconds and clears the status bar warning on success.
